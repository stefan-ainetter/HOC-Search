...
class ObjectRenderGameUnknownCategory(ObjectGame):
    """
    ...
    def initialize_game(self, config, ex_config):
        ...
        self.all_mcts_results_dict = {'score_list':[],'obj_id_list':[],'orientation_list': [], 'iteration_list': [],
                                      'deg_rot45_index_list':[],'best_transform_list':[], 'category_list':[]}

        self.current_rot_45deg_index = 0
        self.best_transform = None

        self.loader = ex_config['dataset'] #self.shapenet_rw.get_obj_loader(self.obj_category)
    ...
    def calc_loss_from_proposals(self, prop_seq=None):
        ...
        category_label = self.prop_seq[0].category_label
        base_rotation_idx = self.prop_seq[1].rotation_degree
        print('Category label = '+ str(category_label) + ', Rotation id = ' + str(base_rotation_idx))
        objects_cluster = leaf_prop.objects_cluster["cluster"]

        score = self.calc_render_loss(objects_cluster,base_rotation_idx,category_label)
    ...
    def calc_render_loss(self,objects_cluster,base_rotation_idx,category_label):

        shapenet_id = objects_cluster[0]['mid'].split('/')[-1]

        batch = self.loader[(shapenet_id,base_rotation_idx,category_label)]
