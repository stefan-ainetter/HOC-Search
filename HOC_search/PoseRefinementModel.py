...
class PoseRefineModel(nn.Module):
    def __init__(self, bg_mesh, obj_mesh,
        ...
        # [roll,pitch,yaw,scale_x,scale_y,scale_z,trans_x,trans_y,trans_z]
    ...
    def reset_model(self, obj_mesh_, scale_func_init_,
                    rotate_func_init_,
                    translate_func_init_):
    ...
    def forward(self):
        ...
        # 3)  base rotation (orientation + scene rot)
        # 4) rotation offset
        rot_func_offset = Transform3d(device=self.device).rotate_axis_angle(angle=roll, axis='X',
                                                                            degrees=False).rotate_axis_angle(
            angle=pitch, axis='Y', degrees=False).rotate_axis_angle(angle=yaw, axis='Z', degrees=False)
        ...
        loss_sil, loss_depth, loss_sensor = loss_pose_refine(mask_pred, self.mask_GT, self.depth_GT,
                                                             ...
                                                             self.mask_depth_valid_render_pred,
                                                             self.device)
